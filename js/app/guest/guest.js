...
export const guest = (() => {
    ...
    const buildGoogleCalendar = () => {
        ...
        const data = new URLSearchParams({
            action: 'TEMPLATE',
            text: 'The Wedding of Chandro and Chrystin',
            dates: `${formatDate('2026-07-13 09:00')}/${formatDate('2026-07-13 12:00')}`,
            details: 'Tanpa mengurangi rasa hormat, kami mengundang Anda untuk berkenan menghadiri acara pernikahan kami. Terima kasih atas perhatian dan doa restu Anda, yang menjadi kebahagiaan serta kehormatan besar bagi kami.',
            location: '[Alamat Lengkap Tempat Acara]',
            ctz: config.get('tz'),
        });
    ...
    const init = () => {
        ...
        window.addEventListener('load', () => {
            if (window.caches && !localStorage.getItem('cache_reset_v8')) {
                window.caches.keys().then((keys) => {
                    return Promise.all(keys.map((k) => window.caches.delete(k)));
                }).then(() => {
                    localStorage.setItem('cache_reset_v8', 'true');
